package quant

import "math"

// FusedDotProduct computes the dot product of a quantized vector and a float32
// vector directly, without allocating a temporary dequantized buffer.
// This is the key optimization: instead of dequantize → allocate → dot → free,
// we process blocks in-place and accumulate the dot product.
//
// data: raw quantized bytes for n elements
// ggmlType: GGUF quantization type
// x: float32 vector (length n)
// n: number of elements
//
// Falls back to Dequantize+dot for unsupported types.
func FusedDotProduct(data []byte, ggmlType uint32, x []float32, n int) float32 {
	switch ggmlType {
	case 0: // F32
		return fusedDotF32(data, x, n)
	case 1: // F16
		return fusedDotF16(data, x, n)
	case 2: // Q4_0
		return fusedDotQ4_0(data, x, n)
	case 3: // Q4_1
		return fusedDotQ4_1(data, x, n)
	case 6: // Q5_0
		return fusedDotQ5_0(data, x, n)
	case 7: // Q5_1
		return fusedDotQ5_1(data, x, n)
	case 8: // Q8_0
		return fusedDotQ8_0(data, x, n)
	case 10: // Q2_K
		return fusedDotQ2_K(data, x, n)
	case 11: // Q3_K
		return fusedDotQ3_K(data, x, n)
	case 12: // Q4_K
		return fusedDotQ4_K(data, x, n)
	case 13: // Q5_K
		return fusedDotQ5_K(data, x, n)
	case 14: // Q6_K
		return fusedDotQ6_K(data, x, n)
	default:
		// Fallback: dequantize then dot
		floats, _ := Dequantize(data, ggmlType, n)
		var sum float32
		for i := 0; i < n; i++ {
			sum += floats[i] * x[i]
		}
		return sum
	}
}

// ── F32 ────────────────────────────────────────────────────────

func fusedDotF32(data []byte, x []float32, n int) float32 {
	var sum float32
	for i := 0; i < n; i++ {
		bits := uint32(data[i*4]) | uint32(data[i*4+1])<<8 | uint32(data[i*4+2])<<16 | uint32(data[i*4+3])<<24
		v := math.Float32frombits(bits)
		sum += v * x[i]
	}
	return sum
}

// ── F16 ────────────────────────────────────────────────────────

func fusedDotF16(data []byte, x []float32, n int) float32 {
	var sum float32
	for i := 0; i < n; i++ {
		bits := uint16(data[i*2]) | uint16(data[i*2+1])<<8
		v := float16ToFloat32(bits)
		sum += v * x[i]
	}
	return sum
}

// ── Q4_0: 32 values per 18-byte block ──────────────────────────
// Scale(f16) + 16 nibble bytes. Quant = nibble - 8.

func fusedDotQ4_0(data []byte, x []float32, n int) float32 {
	var sum float32
	numBlocks := n / 32

	for b := 0; b < numBlocks; b++ {
		off := b * 18
		d := float16ToFloat32(uint16(data[off]) | uint16(data[off+1])<<8)
		xp := x[b*32:]

		var fsum float32
		for j := 0; j < 16; j++ {
			qByte := data[off+2+j]
			q0 := float32(int(qByte&0x0F) - 8)
			q1 := float32(int(qByte>>4) - 8)
			fsum += q0 * xp[j]
			fsum += q1 * xp[j+16]
		}
		sum += d * fsum
	}
	return sum
}

// ── Q4_1: 32 values per 20-byte block ──────────────────────────
// Scale(f16) + Min(f16) + 16 nibble bytes. Value = quant * d + m.

func fusedDotQ4_1(data []byte, x []float32, n int) float32 {
	var sum float32
	numBlocks := n / 32

	for b := 0; b < numBlocks; b++ {
		off := b * 20
		d := float16ToFloat32(uint16(data[off]) | uint16(data[off+1])<<8)
		m := float16ToFloat32(uint16(data[off+2]) | uint16(data[off+3])<<8)
		xp := x[b*32:]

		var fsum float32
		var xsum float32 // sum of x values (for the min term)
		for j := 0; j < 16; j++ {
			qByte := data[off+4+j]
			q0 := float32(qByte & 0x0F)
			q1 := float32(qByte >> 4)
			fsum += q0 * xp[j]
			fsum += q1 * xp[j+16]
			xsum += xp[j] + xp[j+16]
		}
		sum += d*fsum + m*xsum
	}
	return sum
}

// ── Q5_0: 32 values per 22-byte block ──────────────────────────
// Scale(f16) + qh(4 bytes, 5th bits) + 16 nibble bytes. Quant = (4bits | 5th<<4) - 16.

func fusedDotQ5_0(data []byte, x []float32, n int) float32 {
	var sum float32
	numBlocks := n / 32

	for b := 0; b < numBlocks; b++ {
		off := b * 22
		d := float16ToFloat32(uint16(data[off]) | uint16(data[off+1])<<8)
		qh := uint32(data[off+2]) | uint32(data[off+3])<<8 |
			uint32(data[off+4])<<16 | uint32(data[off+5])<<24
		xp := x[b*32:]

		var fsum float32
		for j := 0; j < 16; j++ {
			qByte := data[off+6+j]

			q0 := int(qByte & 0x0F)
			if (qh>>uint(j))&1 != 0 {
				q0 |= 0x10
			}

			q1 := int(qByte >> 4)
			if (qh>>uint(j+16))&1 != 0 {
				q1 |= 0x10
			}

			fsum += float32(q0-16) * xp[j]
			fsum += float32(q1-16) * xp[j+16]
		}
		sum += d * fsum
	}
	return sum
}

// ── Q5_1: 32 values per 24-byte block ──────────────────────────
// Scale(f16) + Min(f16) + qh(4 bytes) + 16 nibble bytes. Value = quant*d + m.

func fusedDotQ5_1(data []byte, x []float32, n int) float32 {
	var sum float32
	numBlocks := n / 32

	for b := 0; b < numBlocks; b++ {
		off := b * 24
		d := float16ToFloat32(uint16(data[off]) | uint16(data[off+1])<<8)
		m := float16ToFloat32(uint16(data[off+2]) | uint16(data[off+3])<<8)
		qh := uint32(data[off+4]) | uint32(data[off+5])<<8 |
			uint32(data[off+6])<<16 | uint32(data[off+7])<<24
		xp := x[b*32:]

		var fsum float32
		var xsum float32
		for j := 0; j < 16; j++ {
			qByte := data[off+8+j]

			q0 := int(qByte & 0x0F)
			if (qh>>uint(j))&1 != 0 {
				q0 |= 0x10
			}
			q1 := int(qByte >> 4)
			if (qh>>uint(j+16))&1 != 0 {
				q1 |= 0x10
			}

			fsum += float32(q0) * xp[j]
			fsum += float32(q1) * xp[j+16]
			xsum += xp[j] + xp[j+16]
		}
		sum += d*fsum + m*xsum
	}
	return sum
}

// ── Q8_0: 32 values per 34-byte block ──────────────────────────
// Scale(f16) + 32 int8 quants. Value = quant * d.

func fusedDotQ8_0(data []byte, x []float32, n int) float32 {
	var sum float32
	numBlocks := n / 32

	for b := 0; b < numBlocks; b++ {
		off := b * 34
		d := float16ToFloat32(uint16(data[off]) | uint16(data[off+1])<<8)
		xp := x[b*32:]

		var fsum float32
		for j := 0; j < 32; j++ {
			q := int8(data[off+2+j])
			fsum += float32(q) * xp[j]
		}
		sum += d * fsum
	}
	return sum
}

// ── Q2_K: 256 values per 84-byte block ─────────────────────────
// scales[16] + qs[64] + d(f16) + dmin(f16)
// Value = d*(scale_lo)*quant_2bit - dmin*(scale_hi)

func fusedDotQ2_K(data []byte, x []float32, n int) float32 {
	var sum float32
	numBlocks := n / 256

	for block := 0; block < numBlocks; block++ {
		off := block * 84
		scOff := off
		qOff := off + 16
		d := float16ToFloat32(uint16(data[off+80]) | uint16(data[off+81])<<8)
		dmin := float16ToFloat32(uint16(data[off+82]) | uint16(data[off+83])<<8)
		xp := x[block*256:]

		xi := 0
		is := 0
		for n128 := 0; n128 < 2; n128++ {
			shift := uint(0)
			for j := 0; j < 4; j++ {
				sc := data[scOff+is]
				is++
				dl := d * float32(sc&0xF)
				ml := dmin * float32(sc>>4)
				for l := 0; l < 16; l++ {
					q := (data[qOff+l] >> shift) & 3
					sum += (dl*float32(q) - ml) * xp[xi]
					xi++
				}

				sc = data[scOff+is]
				is++
				dl = d * float32(sc&0xF)
				ml = dmin * float32(sc>>4)
				for l := 0; l < 16; l++ {
					q := (data[qOff+l+16] >> shift) & 3
					sum += (dl*float32(q) - ml) * xp[xi]
					xi++
				}
				shift += 2
			}
			qOff += 32
		}
	}
	return sum
}

// ── Q3_K: 256 values per 110-byte block ────────────────────────
// hmask[32] + qs[64] + scales[12] + d(f16)

func fusedDotQ3_K(data []byte, x []float32, n int) float32 {
	var sum float32
	numBlocks := n / 256

	const kmask1 = uint32(0x03030303)
	const kmask2 = uint32(0x0f0f0f0f)

	for block := 0; block < numBlocks; block++ {
		off := block * 110
		dAll := float16ToFloat32(uint16(data[off+108]) | uint16(data[off+109])<<8)

		hmOff := off
		qOff := off + 32
		scOff := off + 96
		xp := x[block*256:]

		// Unpack 16 x 6-bit scales
		var aux [4]uint32
		for i := 0; i < 12; i++ {
			byteIdx := i / 4
			shift := uint((i % 4) * 8)
			aux[byteIdx] |= uint32(data[scOff+i]) << shift
		}
		tmp := aux[2]
		aux[2] = ((aux[0] >> 4) & kmask2) | (((tmp >> 4) & kmask1) << 4)
		aux[3] = ((aux[1] >> 4) & kmask2) | (((tmp >> 6) & kmask1) << 4)
		aux[0] = (aux[0] & kmask2) | (((tmp >> 0) & kmask1) << 4)
		aux[1] = (aux[1] & kmask2) | (((tmp >> 2) & kmask1) << 4)

		var scales [16]int8
		for i := 0; i < 16; i++ {
			scales[i] = int8(byte(aux[i/4] >> uint((i%4)*8)))
		}

		xi := 0
		is := 0
		m := byte(1)
		for n128 := 0; n128 < 2; n128++ {
			shift := uint(0)
			for j := 0; j < 4; j++ {
				dl := dAll * float32(scales[is]-32)
				is++
				for l := 0; l < 16; l++ {
					q2 := int((data[qOff+l] >> shift) & 3)
					hBit := 0
					if data[hmOff+l]&m == 0 {
						hBit = 4
					}
					sum += dl * float32(q2-hBit) * xp[xi]
					xi++
				}

				dl = dAll * float32(scales[is]-32)
				is++
				for l := 0; l < 16; l++ {
					q2 := int((data[qOff+l+16] >> shift) & 3)
					hBit := 0
					if data[hmOff+l+16]&m == 0 {
						hBit = 4
					}
					sum += dl * float32(q2-hBit) * xp[xi]
					xi++
				}
				shift += 2
				m <<= 1
			}
			qOff += 32
		}
	}
	return sum
}

// ── Q4_K: 256 values per 144-byte block ────────────────────────
// d(f16) + dmin(f16) + scales[12] + qs[128]

func fusedDotQ4_K(data []byte, x []float32, n int) float32 {
	var sum float32
	numBlocks := n / 256

	for block := 0; block < numBlocks; block++ {
		blockOff := block * 144

		d := float16ToFloat32(uint16(data[blockOff]) | uint16(data[blockOff+1])<<8)
		dmin := float16ToFloat32(uint16(data[blockOff+2]) | uint16(data[blockOff+3])<<8)

		scalesOff := blockOff + 4
		var sc [8]float32
		var mn [8]float32

		for i := 0; i < 4; i++ {
			sc[i] = d * float32(data[scalesOff+i]&0x3F)
			mn[i] = dmin * float32(data[scalesOff+4+i]&0x3F)
		}
		for i := 0; i < 4; i++ {
			scHi := (data[scalesOff+i] >> 6) & 0x03
			mnHi := (data[scalesOff+4+i] >> 6) & 0x03
			scLo := data[scalesOff+8+i] & 0x0F
			mnLo := (data[scalesOff+8+i] >> 4) & 0x0F
			sc[4+i] = d * float32(scLo|scHi<<4)
			mn[4+i] = dmin * float32(mnLo|mnHi<<4)
		}

		qsOff := blockOff + 16
		xp := x[block*256:]
		xi := 0
		is := 0

		for grp := 0; grp < 4; grp++ {
			d1 := sc[is]
			m1 := mn[is]
			d2 := sc[is+1]
			m2 := mn[is+1]
			qOff := qsOff + grp*32

			for l := 0; l < 32; l++ {
				sum += (d1*float32(data[qOff+l]&0x0F) - m1) * xp[xi]
				xi++
			}
			for l := 0; l < 32; l++ {
				sum += (d2*float32(data[qOff+l]>>4) - m2) * xp[xi]
				xi++
			}
			is += 2
		}
	}
	return sum
}

// ── Q5_K: 256 values per 176-byte block ────────────────────────
// d(f16) + dmin(f16) + scales[12] + qh[32] + qs[128]

func fusedDotQ5_K(data []byte, x []float32, n int) float32 {
	var sum float32
	numBlocks := n / 256

	for block := 0; block < numBlocks; block++ {
		blockOff := block * 176

		d := float16ToFloat32(uint16(data[blockOff]) | uint16(data[blockOff+1])<<8)
		dmin := float16ToFloat32(uint16(data[blockOff+2]) | uint16(data[blockOff+3])<<8)

		scalesOff := blockOff + 4
		var sc [8]float32
		var mn [8]float32

		for i := 0; i < 4; i++ {
			sc[i] = d * float32(data[scalesOff+i]&0x3F)
			mn[i] = dmin * float32(data[scalesOff+4+i]&0x3F)
		}
		for i := 0; i < 4; i++ {
			scHi := (data[scalesOff+i] >> 6) & 0x03
			mnHi := (data[scalesOff+4+i] >> 6) & 0x03
			scLo := data[scalesOff+8+i] & 0x0F
			mnLo := (data[scalesOff+8+i] >> 4) & 0x0F
			sc[4+i] = d * float32(scLo|scHi<<4)
			mn[4+i] = dmin * float32(mnLo|mnHi<<4)
		}

		qhOff := blockOff + 16
		qsOff := blockOff + 48
		xp := x[block*256:]
		xi := 0
		is := 0
		u1 := byte(1)
		u2 := byte(2)

		for grp := 0; grp < 4; grp++ {
			d1 := sc[is]
			m1 := mn[is]
			d2 := sc[is+1]
			m2 := mn[is+1]
			qlOff := qsOff + grp*32

			for l := 0; l < 32; l++ {
				q := int(data[qlOff+l] & 0x0F)
				if data[qhOff+l]&u1 != 0 {
					q |= 16
				}
				sum += (float32(q)*d1 - m1) * xp[xi]
				xi++
			}
			for l := 0; l < 32; l++ {
				q := int(data[qlOff+l] >> 4)
				if data[qhOff+l]&u2 != 0 {
					q |= 16
				}
				sum += (float32(q)*d2 - m2) * xp[xi]
				xi++
			}
			is += 2
			u1 <<= 2
			u2 <<= 2
		}
	}
	return sum
}

// ── Q6_K: 256 values per 210-byte block ────────────────────────
// ql[128] + qh[64] + scales[16] + d(f16)

func fusedDotQ6_K(data []byte, x []float32, n int) float32 {
	var sum float32
	numBlocks := n / 256

	for block := 0; block < numBlocks; block++ {
		blockOff := block * 210

		d := float16ToFloat32(uint16(data[blockOff+208]) | uint16(data[blockOff+209])<<8)
		qlBase := blockOff
		qhBase := blockOff + 128
		scBase := blockOff + 192
		xp := x[block*256:]

		for n128 := 0; n128 < 2; n128++ {
			qlOff := qlBase + n128*64
			qhOff := qhBase + n128*32
			xOff := n128 * 128

			for l := 0; l < 32; l++ {
				qlByte0 := data[qlOff+l]
				qlByte32 := data[qlOff+l+32]
				qhByte := data[qhOff+l]

				q1 := (int(qlByte0&0x0F) | (int((qhByte>>0)&3) << 4)) - 32
				q2 := (int(qlByte32&0x0F) | (int((qhByte>>2)&3) << 4)) - 32
				q3 := (int(qlByte0>>4) | (int((qhByte>>4)&3) << 4)) - 32
				q4 := (int(qlByte32>>4) | (int((qhByte>>6)&3) << 4)) - 32

				is := 8*n128 + l/16

				sc0 := float32(int8(data[scBase+is]))
				sc2 := float32(int8(data[scBase+is+2]))
				sc4 := float32(int8(data[scBase+is+4]))
				sc6 := float32(int8(data[scBase+is+6]))

				sum += d * sc0 * float32(q1) * xp[xOff+l]
				sum += d * sc2 * float32(q2) * xp[xOff+l+32]
				sum += d * sc4 * float32(q3) * xp[xOff+l+64]
				sum += d * sc6 * float32(q4) * xp[xOff+l+96]
			}
		}
	}
	return sum
}
